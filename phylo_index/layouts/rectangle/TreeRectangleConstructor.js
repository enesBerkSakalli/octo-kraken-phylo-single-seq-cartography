...
export class TreeConstructor {
   ...
   * @return {void}
   */
  calcBranchLengthDepth(node, radius = 0) {
    ...
    if (node.children) {
      node.children.forEach((child) => {
        this.calcBranchLengthDepth(child, node.radius);
      });
    }
  }
   ...
   * @return {root}
   */
  constructRectangleTree() {
    this.root.data.length = 0;

    this.traverse(this.root);

    this.calcBranchLengthDepth(this.root, 0);

    this.calcAngle(
      this.root,
      this.root.leaves().length * 20,
      this.root.leaves().length
    );
    ...
    this.scale = this.calcScale(this.root.leaves().length * 20, maxRadius, 0.5);
...
export default function constructTree(
  ...
  let treeConstructor = new TreeConstructor(
    d3.hierarchy(tree),
    ignoreBranchLengths
  );
  ...
  let margin = width < height ? width * 0.2 : height * 0.2;
  ...
  let root_ = treeConstructor.constructRectangleTree();
