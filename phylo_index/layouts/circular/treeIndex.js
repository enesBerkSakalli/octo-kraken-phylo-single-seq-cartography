...
export function makeBezierLinks(links, leaves, scene, tree, cosmos, universeId, colorScale) {
    ...
    leaves.forEach(leaf => {
        leafCountPerLink.set(leaf.parentId, (leafCountPerLink.get(leaf.parentId) || 0) + 1);
    });
...
export function makeCollapsedNodes(nodes, scene, tree, cosmos, colorScale, universeId) {
...
export function createAndAddCollapsedTreeElementsToScene(tree, universeId, cosmos, colorScale) {
    let links = tree.links();
    let leaves = tree.leaves();
    let nodes = tree.descendants();
    let universe = cosmos.getUniverseById(universeId);
    makeBezierLinks(links, leaves, universe.scene, tree, cosmos, universeId, colorScale);
    makeCollapsedNodes(nodes, universe.scene, tree, cosmos, colorScale, universeId);
}

export function createAndAddTreeElementsToScene(tree, cosmos, universeId, colorScale) {
    let links = tree.links();
    let leaves = tree.leaves();
    let nodes = tree.descendants();
    let universe = cosmos.getUniverseById(universeId);
    ...
    nodes.forEach((descendant) => {
        leavesMeshArray.forEach((leaveMesh) => {
            if (leaveMesh.leave.uid === descendant.uid) {
                ...
                const scale = 1.30; // This scale factor controls the size of the border

                const borderGeometry = leaveMesh.geometry.clone().scale(scale, scale, scale);
...
export function initializeNodeSelectionAndHighlighting(cosmos, colorScale, universeId) {
...
function guiNodeEvent(event, node, treeUniverse) {
    ...
    gui.add({
        button: async () => {
            ...
            let fullTreeData = deepCopyJSON(await getData(db, 'random_generated_tree'));
            ...
            let treeDisplay = new TreeDisplay(subTree, subTree.maxRadius, 'application');
    ...
    let obj = {
        ...
        add: async () => {
            ...
            let multipleSequenceAlignmentUniverse = new WinBox({ 'title': 'Multiple Sequence Alignment', 'width': '75%', 'height': '75%' });
            ...
            // Try to get the tree data from IndexedDB
            let multipleSequenceAlignment = await getData(db, 'random_generated_tree_msa');

            const filteredList = multipleSequenceAlignment.filter(obj => {
                return node.descendants().some(descendant => descendant.data.name === obj.id);
            });
...
async function initTreeDimensionGUI(cosmos) {
    ...
    function updatePositions() {
        let nodesMeshArray = cosmos.getUniverseById('tree-plot').getMeshesFromContainer('nodes');
        let linksMeshArray = cosmos.getUniverseById('tree-plot').getMeshesFromContainer('links');
        ...
        nodesMeshArray.forEach(mesh => {
            ...
            if (mesh.node.children) {
                ...
                mesh.position.x = (tree.maxRadius) * Math.cos(mesh.node.angle) * scaleFactors.x;
                mesh.position.y = (tree.maxRadius) * Math.sin(mesh.node.angle) * scaleFactors.y;
                mesh.position.z *= scaleFactors.z;
            }
        });
    ...
    function onButtonPressed(key) {
        ...
        let height = cosmos.getUniverseById('tree-plot').container.getBoundingClientRect().height;

        nodesMeshArray.forEach(mesh => {
            mesh.position.z = mesh.node.data.values[key] * (height / highestValueForKey[key].value);
        });
        ...
        linksMeshArray.forEach(mesh => {
            ...
            if (geometry && geometry.attributes.position) {
                let positions = geometry.attributes.position.array;
                positions[2] = mesh.sourceNode.data.values[key] * (height / highestValueForKey[key].value);  // New start z-coordinate
                positions[5] = mesh.targetNode.data.values[key] * (height / highestValueForKey[key].value);  // New end z-coordinate
                geometry.attributes.position.needsUpdate = true;
            }
        });
    }
    ...
    const numericValueKeys = Object.keys(values).filter(key =>
        !isNaN(parseFloat(values[key])) && isFinite(values[key])
    );

    // Add radio buttons to GUI for choosing the key for the Z-axis
    let currentZProperty = { key: 'depth' };  // Default property for Z-axis
    gui.add(currentZProperty, 'key', numericValueKeys).name('Z-Axis Property').onChange(onButtonPressed);
    return gui;
}


export function initializeTreeDimensionPlot(cosmos, treeUniverseId, direction) {
    ...
    let gui = ""
    cosmos.registerUniverse(treeUniverseId, new Universe(camera, renderer, container, controls, scene, gui, window));
...
function findHighestValues(nodes, specificKey = null) {
    ...
    return specificKey ? { [specificKey]: highestValues[specificKey] } : highestValues;
}
